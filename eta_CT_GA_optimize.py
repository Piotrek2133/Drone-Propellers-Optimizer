# Optimization of drone propellers parameters for the best efficiency for given advance ratio
...
# Read model
filename = "eta_cT_model.sav"
model = pickle.load(open(filename, 'rb'))

# Assuming `predict_efficiency` is a function that takes the current c/R and beta values and predicts efficiency
def predict_efficiency(cR_beta_array):
    cR = cR_beta_array[:,0]
...
def evaluate(individual):
    ...
    efficiency = predict_efficiency(cR_beta_array)
